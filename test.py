...
start = time.process_time()
x1, iterates1, residuals1 = cg.m_pcg(A, b, x0, x_exact, preconditioner=jacobi, max_iter=1000, eps=1e-10)
print(x1.dtype)
print(time.process_time() - start, len(iterates1))
print('residual norm:', np.linalg.norm(A @ x1 - b))

start = time.process_time()
#x2, iterates2, residuals2 = cg.pr_pcg(A, b, x0, x_exact, preconditioner=jacobi, max_iter=1000, eps=1e-8, recompute=True)
x2, iterates2, residuals2 = cg.pr_master_pcg(A, b, x0, preconditioner=jacobi, max_iter=1000, eps=1e-10)
print(time.process_time() - start, len(iterates2))
print('residual norm:', np.linalg.norm(A @ x2 - b))

e1_2norm = np.linalg.norm(iterates1 - x_exact, axis=1)
e2_2norm = np.linalg.norm(iterates2 - x_exact, axis=1)
...
plt.semilogy(np.arange(1,len(e1_2norm)+1), e1_2norm, label='error1')
plt.semilogy(np.arange(1,len(e2_2norm)+1), e2_2norm, label='error2')
